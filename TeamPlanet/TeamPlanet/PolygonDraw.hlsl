...
SamplerState	samLinear	: register(s0);//テクスチャのサンプラ

//頂点シェーダ
//引数	 vertexIn : CPUから受け取る頂点情報
//戻り値 vertexOut: PSに送る情報
//頂点情報を(テクスチャサイズ/ウィンドウサイズ)で％を求めて乗算
...
float4 ps(vertexOut IN) : SV_Target
{
	...
	{
		//colにテクスチャの色合成
		float4 tex = txDiffuse.Sample(samLinear, IN.uv);//UVテクスチャの色の値を取得
		col *= tex;
	}
	else
	{
